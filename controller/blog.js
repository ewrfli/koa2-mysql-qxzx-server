...
const findall = async ctx => {
  ...
  //   }
  // })
  const data = await dbConfig.query("SELECT * FROM `blog`", { type: QueryTypes.SELECT }); //原始查询
  ctx.body = {
    code: 200,
    data
  }
...
const list = async ctx => {
  ...
  const { rows: data, count: total } = await Blog.findAndCountAll({ //结合了 findAll 和 count 的便捷方法
