...
const List = async ctx => {
  ...
  const where = {
    category_name: {
      [Op.like]: `%${query.category_name}%`
    }
  }
  console.log('query',query)
  const { rows: data, count: total } = await categoryModel.findAndCountAll({ //结合了 findAll 和 count 的便捷方法
...
const Add = async ctx => {
  ...
  if (!params || !params.category_name) {
    ctx.body = {
      code: 1003,
      msg: '不能为空'
    };
  ...
  try {
    const rel = await categoryModel.create(params);
    ctx.body = {
      code: rel ? 200 : 300,
      msg: rel ? '创建成功' : '创建失败',
      data: rel
...
const Del = async ctx => {
  ...
  const rel = await categoryModel.destroy({ where: ctx.request.body });
  ctx.body = {
    code: rel ? 200 : 300,
    msg: rel ? '删除成功' : '删除失败',
    rel: rel //1成功 0失败
...
const Update = async ctx => {
  ...
  const rel = await categoryModel.update(params, {
